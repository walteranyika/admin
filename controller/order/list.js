angular.module('App').controller('OrderController', function ($rootScope, $scope, $http, $interval,$mdToast, $mdDialog, $cookies, request) {
    ...
    $interval(function() {
     ...
     {
        request.getNumberProducts().then(function (resp) {
            var total_count2 = resp.data.count;
            if(total_count2 == self.total_count)
            {
              console.log("Equal");
              //
            }
	...
	self.loadPages = function () {
        ...
        request.getNumberProducts().then(function (resp) {
            self.total_count = resp.data.count;
